...
namespace CSPN.DAL
{
    public class SystemLogDAL:ISystemLogDAL
    {
        ...
        private const string INSERT_SystemLog = "insert into CSPN_System_Log_Info(Happen_Time,Save_Day,Terminal_ID,Well_State,Temperature,Humidity,Smoke_Detector,Smoke_Power,Signal_Strength) values(@Happen_Time,6,@Terminal_ID,@Well_State,@Temperature,@Humidity,@Smoke_Detector,@Smoke_Power,@Signal_Strength)";
        private const string DELETE_SystemLog = "delete from CSPN_System_Log_Info where Save_Day =6";
        ...
        public int DeleteSystemLogInfo(SystemLogInfo sysLog)
        {
            using (Conn)
            {
                return Conn.Execute(DELETE_SystemLog, sysLog);
            }
        }
    }
}
