...
namespace CSPN.DAL
{
    ...
    public class WellCurrentStateDAL : IWellCurrentStateDAL
    {
        ...
        private const string SELECT_Alarm_Info1 = "select Report_Time,b.Terminal_ID,Well_State_ID,Name,Place,Icon,State,Telephone,RealName from ((CSPN_Well_Current_State_Info as a inner join CSPN_Well_Info as b on a.Terminal_ID=b.Terminal_ID) inner join CSPN_Dic_Well_State_Info as c on a.Well_State_ID=c.ID) inner join CSPN_Operator_Info as d on b.Operator_ID=d.Work_ID where Well_State_ID=2 or Well_State_ID=3 or Well_State_ID=4 or Well_State_ID=5 order by Report_Time desc,Well_State_ID asc";
        private const string SELECT_Alarm_Info2 = "select Report_Time,a.Terminal_ID,Well_State_ID,Name,Place,RealName,Telephone,ReceiveMsg from (CSPN_Well_Current_State_Info as a inner join CSPN_Well_Info as b on a.Terminal_ID=b.Terminal_ID) inner join CSPN_Operator_Info as c on b.Operator_ID=c.Work_ID where Well_State_ID=2 or Well_State_ID=3 or Well_State_ID=4 or Well_State_ID=5 order by Report_Time desc,Well_State_ID asc";
        private const string SELECT_Notice_Info = "select Report_Time,b.Terminal_ID,Well_State_ID,Name,Place,Icon,RealName from ((CSPN_Well_Current_State_Info as a inner join CSPN_Well_Info as b on a.Terminal_ID=b.Terminal_ID) inner join CSPN_Dic_Well_State_Info as c on a.Well_State_ID=c.ID) inner join CSPN_Operator_Info as d on b.Operator_ID=d.Work_ID where Well_State_ID=7 or Well_State_ID=8 order by Report_Time desc";
        private const string SELECT_Well_Current_State_Info = "select Report_Time,Place,RealName,Telephone from (CSPN_Well_Current_State_Info as a inner join CSPN_Well_Info as b on a.Terminal_ID=b.Terminal_ID) inner join CSPN_Operator_Info as c on b.Operator_ID=c.Work_ID where Well_State_ID=@Well_State_ID and a.Terminal_ID=@Terminal_ID";
        ...
        private const string UPDATE_Well_State_ID1 = "update CSPN_Well_Current_State_Info set Well_State_ID=@Well_State_ID where Terminal_ID=@Terminal_ID";
        private const string UPDATE_Well_State_ID2 = "update CSPN_Well_Current_State_Info set Electricity='终端电量正常',Smoke_Detector='烟感正常',Smoke_Power='烟感电量正常',Well_State_ID=@Well_State_ID where Terminal_ID=@Terminal_ID";
        ...
        public int UpdateWellCurrentStateInfo(int well_State_ID, string terminal_ID)
        {
            ...
            {
                if (well_State_ID == 1)
                    return Conn.Execute(UPDATE_Well_State_ID2, new { Well_State_ID = well_State_ID, Terminal_ID = terminal_ID });
                else
                    return Conn.Execute(UPDATE_Well_State_ID1, new { Well_State_ID = well_State_ID, Terminal_ID = terminal_ID });
            }
        }
