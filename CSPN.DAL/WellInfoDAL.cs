...
namespace CSPN.DAL
{
    ...
    public class WellInfoDAL : IWellInfoDAL
    {
        ...
        private const string select_WellInfo_Table = "select a.Terminal_ID,Name,Longitude,Latitude,Place,Terminal_Phone,Well_State_ID,Electricity,Temperature,Humidity,Smoke_Detector,Smoke_Power,Signal_Strength,Icon,Color,ReportInterval,Work_ID,RealName,Telephone from (((CSPN_Well_Info as a inner join CSPN_Operator_Info as b on a.Operator_ID=b.Work_ID) inner join CSPN_Well_Current_State_Info as c on a.Terminal_ID=c.Terminal_ID) inner join CSPN_Dic_Well_State_Info as d on c.Well_State_ID=d.ID) inner join CSPN_Report_Info as e on a.Terminal_ID=e.Terminal_ID where a.Terminal_ID between (select max(a.Terminal_ID) from (select top {0} a.Terminal_ID from CSPN_Well_Info as a order by a.Terminal_ID asc)) and (select max(a.Terminal_ID) from (select top {1} a.Terminal_ID from CSPN_Well_Info as a order by a.Terminal_ID asc)) order by a.Terminal_ID asc";
        private const string select_WellInfo_Count = "select count(*) from CSPN_Well_Info where 1=1";
        private const string select_WellInfo = "select a.Terminal_ID,Name,Longitude,Latitude,Place,Terminal_Phone,c.Terminal_ID as cTerminal_ID,Well_State_ID,Electricity,Temperature,Humidity,Smoke_Detector,Smoke_Power,Signal_Strength,d.ID,Icon,Color,e.Terminal_ID as eTerminal_ID,ReportInterval,Work_ID,RealName,Telephone from (((CSPN_Well_Info as a inner join CSPN_Operator_Info as b on a.Operator_ID=b.Work_ID) inner join CSPN_Well_Current_State_Info as c on a.Terminal_ID=c.Terminal_ID) inner join CSPN_Dic_Well_State_Info as d on c.Well_State_ID=d.ID) inner join CSPN_Report_Info as e on a.Terminal_ID=e.Terminal_ID where 1=1";
        private const string query_WellInfo_Terminal_ID = "select a.Terminal_ID as Terminal_ID,Name,Longitude,Latitude,Place,Terminal_Phone,[State],RealName,e.Terminal_ID,ReportInterval from (((CSPN_Well_Info as a inner join CSPN_Well_Current_State_Info as b on a.Terminal_ID=b.Terminal_ID) inner join CSPN_Dic_Well_State_Info as c on b.Well_State_ID=c.ID) inner join CSPN_Operator_Info as d on a.Operator_ID=d.Work_ID) inner join CSPN_Report_Info as e on a.Terminal_ID=e.Terminal_ID where a.Terminal_ID=@Terminal_ID";
        private const string query_WellInfo_Terminal_Phone = "select a.*,b.Well_State_ID from CSPN_Well_Info as a inner join CSPN_Well_Current_State_Info as b on a.Terminal_ID=b.Terminal_ID where Terminal_Phone=@Terminal_Phone";
        private const string update_WellInfo_Terminal_ID = "update CSPN_Well_Info set Name=@Name,Longitude=@Longitude,Latitude=@Latitude,Place=@Place,Operator_ID=@Operator_ID,Terminal_Phone=@Terminal_Phone where Terminal_ID=@Terminal_ID";
        private const string Insert_WellInfo = "insert into CSPN_Well_Info(Terminal_ID,Name,Longitude,Latitude,Place,Operator_ID,Terminal_Phone) values(@Terminal_ID,@Name,@Longitude,@Latitude,@Place,@Operator_ID,@Terminal_Phone)";
        private const string Delete_WellInfo = "delete from CSPN_Well_Info where Terminal_ID=@Terminal_ID";
