...
namespace CSPN.DAL
{
    ...
    public class WellInfoDAL :IWellInfoDAL
    {
        ...
        const string select_WellInfo = "select a.Terminal_ID,Name,Longitude,Latitude,Place,Terminal_Phone,c.Terminal_ID as cTerminal_ID,Well_State_ID,Temperature,Humidity,Smoke_Detector,Smoke_Power,Signal_Strength,d.ID,Icon,Color,Work_ID,RealName,Telephone from ((CSPN_Well_Info as a inner join CSPN_Operator_Info as b on a.Operator_ID=b.ID) inner join CSPN_Well_Current_State_Info as c on a.Terminal_ID=c.Terminal_ID) inner join CSPN_Dic_Well_State_Info as d on c.Well_State_ID=d.ID where 1=1";
